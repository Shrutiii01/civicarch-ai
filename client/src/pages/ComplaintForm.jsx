...
export default function ComplaintForm() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const formData = new FormData();
      // 🔥 AI Hint: Forces the backend to process this specifically as a Complaint
      formData.append("text", `[USER INTENT: FORMAL CIVIC COMPLAINT]\n${text}`);
      formData.append("location", location);
      formData.append("pincode", pincode);
      formData.append("category", category || "complaint"); // Fallback to complaint if empty
 ...
 {/* ── Navbar ── */}
      ...
      <section className="bg-[#111] text-white py-16 text-center">
        <h1 className="text-4xl font-serif font-bold mb-4">Formal Complaint</h1>
        <p className="text-stone-400 text-sm max-w-2xl mx-auto font-light">
          Report physical issues like potholes, waste, or broken infrastructure for immediate municipal action.
        </p>
      </section>
      ...
      <main className="flex flex-1 flex-col lg:flex-row gap-8 px-6 py-8 lg:px-20 max-w-[1600px] mx-auto w-full">
        ...
        <section className="flex-1 flex flex-col gap-6">
          <div className="space-y-1">
            <h1 className="text-3xl font-bold font-serif">Lodge a Complaint</h1>
            <p className="text-slate-500">Our AI Architect will analyze your complaint for legal compliance and municipal routing.</p>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden p-6">
            ...
            <div className="flex flex-wrap gap-3 mb-8 pb-6 border-b border-slate-100">
              ...
              <div className="flex items-center gap-2 px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-slate-500 text-xs font-semibold uppercase tracking-wider cursor-default select-none shadow-sm">
                <span className="material-symbols-outlined text-[16px] text-red-600">mic</span> Voice Translation
              </div>
            </div>

            <div className="space-y-6">
              <div className="relative">
                <label className="block text-sm font-semibold text-slate-700 mb-2">Description of Violation/Issue</label>

                {/* Textarea Wrapper with Absolute Positioned Mic Button */}
                <div className="relative">
                  <textarea
                    className="w-full min-h-[280px] p-5 pb-20 rounded-lg border border-slate-200 bg-slate-50 focus:ring-2 focus:ring-red-600 focus:border-transparent text-lg resize-none outline-none transition-all"
                    placeholder="Describe the physical issue in detail or click the mic to speak in any language..."
                    value={displayText}
                  ...
                  <button
                    ...
                    className={`absolute bottom-5 right-5 flex items-center justify-center h-14 w-14 rounded-full shadow-lg transition-all ${isRecording
                        ...
                        : "bg-white border-2 border-slate-200 text-slate-600 hover:text-red-600 hover:border-red-600 shadow-sm"
                      }`}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <input
                      className="w-full pl-10 pr-4 py-3 rounded-lg border border-slate-200 bg-slate-50 focus:ring-2 focus:ring-red-600 outline-none"
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input
                    className="w-full pl-10 pr-4 py-3 rounded-lg border border-slate-200 bg-slate-50 focus:ring-2 focus:ring-red-600 outline-none"
                    placeholder="Road / Sanitation / Electricity etc."
                    type="text"
                    value={category}
                ...
                >
                  {preview ? (
                    ...
                    <div className="text-center">
                      <span className="material-symbols-outlined text-red-600 text-3xl mb-1">upload_file</span>
              ...
              >
                ...
                {loading ? "Processing Input / Submitting..." : "Generate Complaint Draft"}
              </button>
            </div>
          </div>
        </section>
        ...
        <aside className="w-full lg:w-[380px] space-y-6 pt-6 lg:pt-14">
          ...
          {/* Card 1: What is a Complaint? */}
          <div className="bg-white rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-stone-100 p-6 relative overflow-hidden">
            <div className="absolute top-0 left-0 w-full h-32 bg-gradient-to-b from-red-50 to-white pointer-events-none"></div>
            <div className="relative z-10">
              <h3 className="font-bold text-lg text-slate-800 flex items-center gap-3 mb-4">
                <div className="flex items-center justify-center h-6 w-6 rounded-full border border-red-600 text-red-600">
                  <span className="material-symbols-outlined text-[16px]">info</span>
                </div>
                Formal Complaint
              </h3>
              <p className="text-sm text-stone-500 mb-6 leading-relaxed">
                A formal complaint is used to report physical civic violations, broken infrastructure, or public safety hazards that require immediate municipal action.
              </p>
              <ul className="space-y-4">
                <li className="flex items-start gap-3 text-sm text-stone-600 font-medium">
                  ...
                  <span className="leading-snug">Requires precise geographical details for fast dispatch.</span>
                </li>
                <li className="flex items-start gap-3 text-sm text-stone-600 font-medium">
                  <span className="material-symbols-outlined text-red-500 text-[20px]">check_circle</span>
                  <span className="leading-snug">Routed to Municipal Corporations or the PWD.</span>
                </li>
              </ul>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-stone-100 p-6">
            ...
            <ul className="space-y-3 pt-1">
              {[
                "Deep potholes causing traffic hazards",
                "Open manholes or broken sewage lines",
                "Illegal garbage dumping in public",
                "Fallen trees blocking main roads",
                "Broken streetlights in residential areas"
              ].map((issue, idx) => (
                <li key={idx} className="flex items-center gap-3 text-sm text-stone-600">
                  <div className="h-1.5 w-1.5 rounded-full bg-red-500/60"></div>
                  {issue}
                </li>
              ))}
            </ul>
